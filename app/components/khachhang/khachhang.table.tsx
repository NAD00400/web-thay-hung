...
interface IKhachHang {
  ...
  ma_nguoi_dung: string; // Mã người dùng
        ...
        <thead className="bg-gray-100">
          <tr>
            {["Tên Khách Hàng", "Số Điện Thoại", "Địa Chỉ", "Email Người Dùng", "Hành Động"].map((header) => (
              <th key={header} className="px-4 py-3 text-left font-semibold  border-b border-neutral-200 text-gray-600">
                {header}
              </th>
            ))}
          </tr>
          ...
          {dataKH && dataKH.length > 0 ? (
            dataKH.map((khachhang) => (
                ...
                <tr className="border-b border-neutral-200 bg-white hover:bg-gray-100 transition">
                  <td className="px-4 py-3">{khachhang.ten_khach_hang}</td>
                  ...
                  <button
                    ...
                    className="text-gray-600 hover:text-gray-800 transition focus:outline-none focus:ring-2 focus:ring-blue-500 rounded"
